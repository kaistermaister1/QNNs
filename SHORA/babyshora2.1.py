...
# Feature map - Minimal Nonlinearity for Linear Separability  
# ------------------------------------------------------------------
# 1) Linear encoding (preserve all input information)
for i in range(n_qubits):
    qc.ry(np.pi * input_params[i], i)

# ------------------------------------------------------------------
# 2) Add just enough nonlinearity via pairwise products
qc.rzz(np.pi * input_params[0] * input_params[1], 0, 1)  # Even/odd + multiples of 3 interactions
qc.rzz(np.pi * input_params[2] * input_params[3], 2, 3)  # Higher bit interactions

# ------------------------------------------------------------------  
# 3) Simple entanglement to mix the features
qc.cx(0, 1)
qc.cx(2, 3)
qc.cx(1, 2)

qc.barrier(label="Feature map done")
...
plt.savefig("plots/babyshora2.1circuit.png", dpi=150, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig("plots/babyshora2.1.png", dpi=150, bbox_inches='tight')
plt.show()

print(f"\nPlots saved to plots/babyshora2.1.png")
