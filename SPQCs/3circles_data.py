...
def create_3circles_boundaries(centers, radii):
...
def generate_3circles_dataset(num_points=1000, centers=None, radii=None):
    """Generate random points with balanced inside/outside distribution for 3 circles."""
    np.random.seed(42)  # For reproducibility
    if centers is None:
        centers = [(0.3, 0.3), (0.7, 0.3), (0.5, 0.7)]
    if radii is None:
        radii = [0.15, 0.12, 0.13]
    boundaries = create_3circles_boundaries(centers, radii)
...
def create_labeled_dataset(num_points=100):
    ...
    inside_points, outside_points, boundaries = generate_3circles_dataset(num_points)
...
def visualize_data(num_points=100):
    """Visualize the 3circles dataset with train/test split."""
    X_train, X_test, y_train, y_test, boundaries = get_3circles_data(num_points)
...
def get_3circles_data(num_points=100):
    """
    Get all 3circles classification data without plotting.
    Args:
    ...
    Label encoding:
        - Inside any blob: 0
        - Outside all 3circles: 1
    """
    return create_labeled_dataset(num_points)

if __name__ == "__main__":
    print("Demonstrating 3circles dataset:")
    print("=" * 50)
    sizes_to_test = [50, 100, 200, 500]
    for size in sizes_to_test:
        print(f"\nTesting with {size} total points:")
        X_train, X_test, y_train, y_test, boundaries = get_3circles_data(size)
    ...
    visualize_data(200)
    X_train, X_test, y_train, y_test, boundaries = get_3circles_data(200)
    print(f"\nExample data points:")
    ...
    print(f"Outside all 3circles: 1")
    print(f"\nTo use custom dataset sizes in your code:")
    print(f"train_features, test_features, train_labels, test_labels, boundaries = get_3circles_data(500)") 