...
def create_square_boundary(center=(0.5, 0.5), side_length=0.4):
    """Create a square boundary using Path."""
...
def generate_dataset(num_points=2000):
    ...
    square_path = create_square_boundary()
    ...
    # Check which points are inside the square
    inside_mask = square_path.contains_points(candidate_points)
    ...
    while len(inside_points) < num_inside or len(outside_points) < num_outside:
        ...
        additional_inside_mask = square_path.contains_points(additional_points)
    ...
    return inside_points, outside_points, square_path


def create_labeled_dataset(num_points=100):
    """Create labeled dataset with scalar labels (0 or 1) and 80/20 split."""
    inside_points, outside_points, square_path = generate_dataset(num_points)
    ...
    return X_train, X_test, y_train, y_test, square_path

def visualize_data(num_points=100, data=None):
    """Visualize the square boundary dataset with train/test split."""
    if data is not None:
        X_train, X_test, y_train, y_test, square_path = data
    else:
        X_train, X_test, y_train, y_test, square_path = get_square_data(num_points)
    ...
    vertices = square_path.vertices
    square_x = vertices[:, 0]
    square_y = vertices[:, 1]
    plt.plot(square_x, square_y, 'k:', linewidth=3, label='Square boundary')
...
def get_square_data(num_points=100):
    """
    Get all square classification data without plotting.
    
    Args:
        num_points: Total number of data points to generate (default: 100)
    
    Returns:
        tuple: (X_train, X_test, y_train, y_test, square_path)
            ...
            - square_path: The square boundary path object
...
if __name__ == "__main__":
    # Generate and visualize square dataset
    print("Demonstrating square dataset:")
    print("=" * 50)
    ...
    for size in sizes_to_test:
        ...
        X_train, X_test, y_train, y_test, square_path = get_square_data(size)
    ...
    print(f"\nLabel encoding:")
    print(f"Inside square: 0")
    print(f"Outside square: 1")
    print(f"\nTo use custom dataset sizes in your code:")
    print(f"train_features, test_features, train_labels, test_labels, square_path = get_square_data(500)")